...
public class VComposition implements TextBlock {
  ...
  public VComposition(TextBlock top, TextBlock bottom) {
    if (top == null) { // handles null case
        this.top = new TextLine("");
    } else {
        this.top = top;
    }
  
    if (bottom == null) { // handles null calse
 ...
 public boolean eqv(TextBlock cmp) {
    cmp = (VComposition) cmp;
    return (((cmp instanceof VComposition) && this.top.eqv((((VComposition) cmp).top))) // checks the left path then right then calls both recursively
            && this.bottom.eqv((((VComposition) cmp).bottom)));
}
