...
public class HComposition implements TextBlock {
  ...
  public HComposition(TextBlock left, TextBlock right) {
    if (left == null) { // handles null case
      this.left = new TextLine("");
    } else {
      this.left = left;
    }

    if (right == null) { // handles null calse
  ...
  public boolean eqv(TextBlock cmp) {
    cmp = (HComposition) cmp;
    return (((cmp instanceof HComposition) && this.left.eqv((((HComposition) cmp).left))) // checks the left path then right then calls both recursively
            && this.right.eqv((((HComposition) cmp).right)));
}
